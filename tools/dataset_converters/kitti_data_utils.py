...
def get_kitti_image_info(path,
    ...
    if not isinstance(image_ids, list):
        image_ids = list(range(image_ids))   # 记录id列表
    ...
    with futures.ThreadPoolExecutor(num_worker) as executor:
        # 迭代的字典 本来是一帧的标注，最后返回的一个所有的帧的标注
        image_infos = executor.map(map_func, image_ids)
       
        # for my_dict in image_infos:
        #     print(my_dict)
        #     sys.exit()
            
    return list(image_infos)  # 最后返回的一个所有的帧的标注 组成一个list
