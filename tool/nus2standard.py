...
save_file = "/home/lin/code/mmdetection3d/data/custom"
...
for sample in nusc.sample:
    ...
    for cam in cameras:
        ...
        imgs.append(img)
        Ks.append(K)
        T_cam_lidar = T_cam_global @ np.linalg.inv(T_lidar_world_new)
    ...
    # meta_data ['width'] = points_.shape[0]    # 使用点云数据的行数作为宽度
    # meta_data ['height'] = 1
    # meta_data ['viewpoint'] = [0, 0, 0, 1, 0, 0, 0]
    # meta_data ['points'] = points_.shape[0]   # 使用点云数据的行数作为点数
